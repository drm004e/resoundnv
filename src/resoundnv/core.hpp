...
public:
	...
	ChaseBehaviour(const xmlpp::Node* node, ResoundSession* session);
	virtual void process(jack_nframes_t nframes);
	static Behaviour* factory(const xmlpp::Node* node, ResoundSession* session) { return new ChaseBehaviour(node,session); }
};
...
class AmpPanBehaviour : public IOBehaviour {
	...
	float* oldGains_;
public:
	AmpPanBehaviour(const xmlpp::Node* node, ResoundSession* session);
	virtual void process(jack_nframes_t nframes);
	static Behaviour* factory(const xmlpp::Node* node, ResoundSession* session) { return new AmpPanBehaviour(node,session); }
};
